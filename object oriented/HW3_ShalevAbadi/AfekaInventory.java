...
public class AfekaInventory<E extends MusicalInstrument> implements InventoryFunc<E> {
	...
	public void addAllStringInstruments(ArrayList<? extends MusicalInstrument> src,
			ArrayList<? super MusicalInstrument> des) {
		...
		if (isSorted) {
			...
			while (high >= low) {
				...
				if (brand.compareToIgnoreCase(arr.get(mid).getBrand()) < 0)
					high = mid - 1;
				else if (brand.compareToIgnoreCase(arr.get(mid).getBrand()) == 0) {
					if (brandIndex == -1)
						brandIndex = mid;
					else if (Math.abs(price.doubleValue() - arr.get(brandIndex).getPrice().doubleValue()) > Math
							.abs(price.doubleValue() - arr.get(mid).getPrice().doubleValue()))
						brandIndex = mid;
					if ((price.doubleValue() - arr.get(mid).getPrice().doubleValue()) < 0)
						high = mid - 1;
					else if ((price.doubleValue() - arr.get(mid).getPrice().doubleValue()) == 0)
						return mid;
					else
						low = mid + 1;
				} else
